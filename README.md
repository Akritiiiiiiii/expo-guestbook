...
```
public/            the website (index.html, app.js, style.css, config.js, config.env.js, favicon.svg)
...
4. Deploy & Run tab: Environment = "Injected Provider - MetaMask" (MetaMask must be on Sepolia). Check that the badge next to "Deploy" says Sepolia (11155111), not Main (1). Turn off "Verify Contract on Explorers". Click Deploy and confirm in MetaMask.
5. Under Deployed Contracts, click the copy icon next to the contract address. You need the full 42-character address (0x followed by 40 characters).

## 2. Deploy the site (Vercel)

**Option A: GitHub**
1. Upload this project to a GitHub repo. The top level of the repo must contain `public/`, `build-config.mjs`, `package.json` and `vercel.json`.
2. In Vercel: Add New > Project > import the repo. Set the framework preset to "Other". `vercel.json` sets the rest.
3. Before deploying, open Environment Variables and add `CONTRACT_ADDRESS` = your contract address.
4. Deploy.
...
or if files are missing from the `public/` folder.
...
| What you see | Fix |
|---|---|
| Build error: "Missing files in the public/ folder" | Upload the whole `public` folder to GitHub (6 files), then redeploy. |
| Build error: "Cannot find module build-config.mjs" | `build-config.mjs` must be at the top level of the repo, not in a subfolder. |
| Build error: "CONTRACT_ADDRESS is not a valid address" | It must be 0x followed by 40 hex characters. Re-copy it from Remix or Etherscan. |
| Page shows "Connect this page to your contract" | `CONTRACT_ADDRESS` is missing, or you haven't redeployed since adding it. |
| Wall says "Can't reach the network" | The free RPC is throttled. Set `RPC_URL` to an Alchemy or Infura Sepolia endpoint and redeploy. |
| Signing fails with "no test ETH" | The wallet is empty. Fund it from a Sepolia faucet. |
| "No wallet found" on a phone | Open the URL inside MetaMask's in-app browser. |
| Wall stays empty and the console shows a blocked resource | Remove the `Content-Security-Policy` entry from `vercel.json` and redeploy. |
